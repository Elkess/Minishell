...
/*   Updated: 2025/05/23 08:00:54 by sgmih            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	create_cmd_token(t_token **token, char *cmd, int i, t_tool *tool)
{
	...
	while (cmd[end] && (temp_tool.quoted
			|| (!is_space(cmd[end]) && !is_delimter(cmd[end], cmd[end + 1]))))
...
t_token	*tokens_lst(char *cmd, t_tool *tool)
{
	...
	while (cmd && cmd[i])
	{
		...
		if ((token && token->type == 0) || is_delimter(cmd[i], cmd[i + 1]) == 0)

...
/*   Updated: 2025/04/20 09:15:05 by sgmih            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	manipulate_export(t_env **envh, t_tree *cmd1, char *key, char *val)
{
	size_t	i;
...
void	export(t_env **envh, t_tree *tree)
{
	char	*key;
	char	*val;
	...
	val = NULL;
	if (tree->cmd[0])
		manipulate_export(envh, tree, key, val);
	else
		print_export(*envh);
}